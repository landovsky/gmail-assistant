...
Admin endpoints require HTTP Basic Auth. Read credentials from env vars `GMA_ADMIN_USER` and `GMA_ADMIN_PASSWORD`.
Use: `curl -u "$GMA_ADMIN_USER:$GMA_ADMIN_PASSWORD"` for all `/admin/` requests.
...
2. Register Gmail watch: `curl -s -u "$GMA_ADMIN_USER:$GMA_ADMIN_PASSWORD" -X POST https://gmail.kopernici.cz/admin/watch`
